...
class Data():
    def __init__(self, data, max_num_sentence, keywords_dic, num_categories, LDA=False):
        ...
        self.inputs = np.asarray(padded_inputs)  # Convert to NumPy array
    ...
    def get_slice(self, iList):
        ...
        for u_input in inputs:
            temp_s = []
            for s in u_input:
                if isinstance(s, np.ndarray):  # Convert NumPy arrays to lists
                    s = s.tolist()
                if len(s) > 0:  # Check if the sequence is not empty
                    temp_s.extend(s)  # Use extend() instead of += for lists
