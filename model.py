...
class DocumentGraph(Module):
    def __init__(self, opt, pre_trained_weight, class_weights, n_node, n_categories,vocab_dic=None):
        ...
        if pre_trained_weight is not None:
          ...
          # Random initialization for datasets without pretrained embeddings
        ...
        if opt.dataset in ['mr', 'Tweet', 'Pascal_Flickr']:
          ...
          # Add explicit dimension check and reshape if needed
          if pre_trained_weight.dim() != 2:
              if pre_trained_weight.dim() == 1:
                  pre_trained_weight = pre_trained_weight.unsqueeze(0)  # Make it 2D
              else:
                  raise ValueError(f"Expected 2D tensor, got {pre_trained_weight.dim()}D input")
          
          # Add vocabulary index verification
          ...
          self.embedding = nn.Embedding.from_pretrained(
              pre_trained_weight,
              freeze=False,
              padding_idx=0  # Must match your weights_matrix[0] initialization
          )
        ...
        if self.class_weights is not None:
            # Add verification and padding if needed
    ...
    def reset_parameters(self):
        ...
        # Add optimizer initialization here instead
